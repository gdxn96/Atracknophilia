...
std::pair<IEntity*, Vector2D> PhysicsSystem::RayCastToDynamicObject(Vector2D start, Vector2D end, float maxLength)
{
	b2RayCastInput input;
	input.p1 = start.toBox2DVector();
	input.p2 = end.toBox2DVector();
	...
	for (b2Body* b = World().GetBodyList(); b; b = b->GetNext()) {
		for (b2Fixture* f = b->GetFixtureList(); f; f = f->GetNext()) {
			...
			if (!f->RayCast(&output, input, 0))
				continue;
			if (output.fraction < closestFraction) {
				auto isDynamic = ((Box2DComponent*)(b->GetUserData()))->getComponent<DynamicBodyComponent>();
				if (!isDynamic)
				{
					continue;
				}
				collidedEntity = isDynamic->getParent();
				closestFraction = output.fraction;
				intersectionNormal = output.normal;
			}
		}
